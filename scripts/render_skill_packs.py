...
def build_files(slug: str, contract: dict, binding: dict | None = None) -> dict[str, str]:
    ...
    positive = [
        ...
        f"Build a rigorous {decision} for this marketing problem",
    ]
    implicit = [
        f"We have conflicting signals and need a decision about {decision}",
        f"I have data but I am not sure what it means for {decision}",
        f"Challenge my assumptions before we finalize {decision}",
    ]
    negative = [
    ...
    if slug == "marketing-council":
        positive = [
            "Diagnose this cross-functional marketing problem before choosing which specialists should own it",
            "Several marketing functions may be causing the result; decide what matters first and coordinate the minimum specialists",
            "Build an integrated marketing decision where positioning, pricing, media, and retention constraints may interact",
        ]
        implicit = [
            "Growth is weak and we do not know whether the issue is market, offer, pricing, media, conversion, or retention",
    ...
    workflow = [
        {
            "id": "frame",
            "action": f"Frame the {decision} and the decision owner for {slug}.",
            ...
            "completion": f"The {decision} is stated as one explicit decision question.",
        },
        {
            "id": "evidence",
            "action": f"Collect and classify the minimum evidence needed for {decision}: {', '.join(evidence)}.",
        ...
        {
            "id": "diagnose",
            "action": f"Test competing explanations for the {decision} instead of accepting the first plausible story.",
        ...
        {
            "id": "decide",
            "action": f"Produce the {decision} with explicit trade-offs, risks, and what would reverse it.",
        ...
        {
            "id": "measure",
            "action": f"Define how the {decision} will be observed, challenged, and handed off.",
    ...
    spec = {
        ...
        "non_goals": [
            ...
            if slug != "marketing-council"
            else "Replacing focused expertise when one skill clearly owns the decision",
        ],
        "workflow": workflow,
        "capabilities": {
    ...
    return {
        "SKILL.md": skill_md,
        "references/skill-spec.json": json.dumps(spec, indent=2, ensure_ascii=False) + "\n",
        "references/decision-model.md": decision_md,
        "references/failure-modes.md": failure_md,
        "references/output-contract.md": output_md,
        "evals/activation.yml": activation_yml,
        "evals/behavior.yml": behavior_yml,
        "evals/pressure.yml": pressure_yml,
        "evals/regression.yml": regression_yml,
    }
...
def check() -> int:
    ...
    print(json.dumps({"ok": not drift, "files": len(expected), "drift": drift}, indent=2))
...
def main() -> None:
    ...
    raise SystemExit(write() if args.write else check())
