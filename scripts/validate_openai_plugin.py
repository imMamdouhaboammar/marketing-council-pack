...
def skill_frontmatter_keys(path: Path) -> set[str]:
    text = path.read_text(encoding="utf-8")
    if not text.startswith("---\n"):
        return set()
    end = text.find("\n---", 4)
...
def openai_skill_interface(path: Path) -> dict[str, str]:
    data: dict[str, str] = {}
...
def validate_plugin(root: Path) -> dict:
    ...
    if skill_root.is_dir():
        for directory in sorted(p for p in skill_root.iterdir() if p.is_dir()):
            ...
            add(errors, skill_frontmatter_keys(definition) == {"name", "description"}, f"unsupported SKILL.md frontmatter fields: {directory.name}")

            openai_yaml = directory / "agents" / "openai.yaml"
            add(errors, openai_yaml.is_file(), f"skill missing agents/openai.yaml: {directory.name}")
            if openai_yaml.is_file():
                skill_iface = openai_skill_interface(openai_yaml)
                add(errors, bool(skill_iface.get("display_name")), f"skill interface.display_name missing: {directory.name}")
                short = skill_iface.get("short_description", "")
                add(errors, 25 <= len(short) <= 64, f"skill interface.short_description must be 25..64 chars: {directory.name}")
                prompt = skill_iface.get("default_prompt", "")
                add(errors, f"${directory.name}" in prompt, f"skill interface.default_prompt must mention ${directory.name}")
