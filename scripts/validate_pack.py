...
def _openai_interface(path: Path) -> dict:
    text = path.read_text(encoding="utf-8")
    data: dict[str, str] = {}
...
def validate_pack(root: str | Path) -> dict:
    ...
    for path in skill_files:
        ...
        if extra_fields:
            errors.append(f"{path}: unsupported SKILL.md frontmatter fields: {sorted(extra_fields)}")
        ...
        if path != root / "SKILL.md":
            ...
            if not openai_path.is_file():
                errors.append(f"{path.parent.name}: missing agents/openai.yaml")
            else:
                iface = _openai_interface(openai_path)
                short = iface.get("short_description", "")
                prompt = iface.get("default_prompt", "")
                if not iface.get("display_name"):
                    errors.append(f"{path.parent.name}: missing interface.display_name")
                if not 25 <= len(short) <= 64:
                    errors.append(f"{path.parent.name}: interface.short_description must be 25..64 characters")
                if f"${path.parent.name}" not in prompt:
                    errors.append(f"{path.parent.name}: interface.default_prompt must mention ${path.parent.name}")
