...
class DistributionTests(unittest.TestCase):
    ...
    def test_every_skill_has_compliant_openai_interface_metadata(self):
        ...
        for skill in skills:
            ...
            self.assertTrue(values.get("display_name"), metadata)
            short = values.get("short_description", "")
            self.assertGreaterEqual(len(short), 25, metadata)
            self.assertLessEqual(len(short), 64, metadata)
            prompt = values.get("default_prompt", "")
            self.assertIn(f"${slug}", prompt, metadata)
            self.assertNotIn("\n", prompt, metadata)
